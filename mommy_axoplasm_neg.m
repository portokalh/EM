function [mybwimg,mygrayimg, initno]=mommy_axoplasm_neg(filein,mag)
%%%%%%%%%%%%%%%%%%%%%%%%
%call as
%[im1,res2]=mommy_axoplasm(filein,7100);
%%%%%%%%%%%%%%%%%%%%%%%%
if mag==2650
    radius_mf=3;
    myarea=2500;
...
switch 1
    case flag==1
        %enhance contrast
        %flag=1
        im11=imadjust(im1,[0.3 1],[]);%try 0.3 ,1; was 0.3 0.7
        ...
        % imTest=(rangefilt(medfilt2(im3,[6 6]),getnhood(strel('disk',12))));
...
figure(3)
title('erode')
mydisk=strel('disk',3); %was 2 %mydisk=strel('disk',4);
...
im_dil=imdilate(imdilate(imdilate(~BW,mydisk),mydisk), mydisk);%was ~BW
end

im_erode=imerode(imerode(imerode(im_dil,mydisk),mydisk), mydisk);

im_erode=~im_erode; %was not here
subplot(1,2,1);
imagesc(im_erode);
title('dilx3,errx3')

cc=bwconncomp(im_erode);%was ~im_erode
...
serode  = regionprops(bw2, 'centroid');%was ~bw2
...
cc=bwconncomp(im_open);%was ~im_open
...
sopen  = regionprops(bw2, 'centroid');%was ~bw2
...
%active snakes?
 figure(5)
   [D,IDX] = bwdist(bw2,'euclidean');%was ~bw2
